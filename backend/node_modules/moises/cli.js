...
cmdProcess
  ...
  .argument('<outputFolder>', 'Destination folder')
  .option('-w, --watch', 'Keep watching folder for new files')
  .option('-c, --concurrency <number>', 'Number of concurrent jobs', '10')
  .action(async (workflow, inputFolder, outputFolder, options) => {
    try {
      ...
      if (isNaN(concurrency) || concurrency < 1 || concurrency > 20) {
        fail("The --concurrency option must be a number between 1 and 20.")
        return
      }
